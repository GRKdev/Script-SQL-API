...
from utils.functions_clients import (
    ...
    generate_telefon_queries,
    generate_email_queries,
    generate_direccio_queries,
    load_names_from_json,
    generate_custom_queries_multi,
    generate_direc_multi,
    generate_email_multi,
    generate_telefon_multi,
    generate_from_telefon,
    generate_phone_numbers,
    generate_todo_clientes,
)
...
class ClientTab:
    def __init__(self, master):
        ...
        self.label_global_train_count = tk.Label(
            self.frame, text="Cantidad Train Global:"
        )
        ...
        self.button_toggle_all = tk.Button(
            self.frame, text="Toggle All", command=self.toggle_all
        )
        ...
        for idx, q_type in enumerate(self.query_types):
            ...
            self.query_checkbuttons[q_type] = tk.Checkbutton(
                self.frame, variable=self.query_checkbutton_vars[q_type]
            )
            self.query_checkbuttons[q_type].grid(row=idx + 2, column=3)

        self.button_generate = tk.Button(
            self.frame, text="Genera el Arxiu", command=self.generate_jsonl
        )
    ...
    def generate_jsonl(self):
        function_names = {
            q_type: entry.get().strip()
            for q_type, entry in self.query_entries.items()
            if self.query_checkbutton_vars[q_type].get()
        }
        default_train_count = (
            int(self.entry_train_count.get())
            if hasattr(self, "entry_train_count")
            else 100
        )

        for query_type, function_name in function_names.items():
            train_count = int(
                self.query_train_counts.get(query_type, default_train_count).get()
            )
            ...
            if query_type == "Telefono_cliente":
                (
                    self.train_prompts_list,
                    self.valid_prompts_list,
                ) = generate_phone_numbers(train_count, valid_count)
            else:
                ...
                self.train_prompts_list = [
                    original_train_prompts[i % len(original_train_prompts)]
                    for i in range(train_count)
                ]
                self.valid_prompts_list = [
                    original_valid_prompts[i % len(original_valid_prompts)]
            ...
            self.generate_file(
                train_filepath, self.train_prompts_list, query_type, function_name
            )
            self.generate_file(
                valid_filepath, self.valid_prompts_list, query_type, function_name
            )
    ...
    def generate_file(self, filepath, prompts_list, query_type, function_name):
        ...
        if query_type in query_types:
            ...
            generate_func(
                generated_lines, table_name, function_name, prompts_list, is_valid
            )
