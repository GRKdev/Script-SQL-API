...
from utils.functions_clients import generate_custom_queries_multi, generate_direc_multi, generate_email_multi,generate_telefon_multi
...
class ClientTab:
    def __init__(self, master):
        ...
        self.query_type = ttk.Combobox(self.frame, values=["Custom", "Telefon", "Email", "Direccion", "Custom_Multi", "Telefon_Multi", "Email_Multi", "Direccion_Multi"], state="readonly")
    ...
    def generate_jsonl(self):
        ...
        # Si estamos en modo de consulta múltiple, duplicamos el conteo
        if self.query_type.get().endswith("_Multi"):
            train_count *= 2

        valid_count = int(train_count * 0.20)  # 20% para validación
