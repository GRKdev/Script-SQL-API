...
from utils.functions_stats import (
    generate_random_years, generate_fact_current_year_client, generate_fact_total_client,
    generate_ing_current_year_client, generate_ing_total_client, generate_fact_total,
    generate_fact_current_year, generate_fact_selected_year, generate_ing_total,
    generate_ing_current_year, generate_ing_selected_year,load_names_from_json
)
...
clientes_predefined = predefined_values['estadisticas']

class EstadisticasTab:
    def __init__(self, master):
        ...
        self.entry_table.insert(0, "alb_stat")
        ...
        self.query_types = [
            "cli_fact_cy", "cliente_fact_total", "cliente_ing_anual", "cliente_ing_total",
            "fact_total", "fact_currenty", "fact_selecty", "ing_total", "ing_currenty", "ing_selecty"
            ]
        self.query_entries = {}
    ...
    def generate_jsonl(self):
        ...
        for query_type, function_name in function_names.items():
            ...
            if query_type in ["fact_selecty", "ing_selecty"]:
                self.train_prompts_list, self.valid_prompts_list = generate_random_years(train_count, valid_count)
            else:
    ...
    def generate_file(self, filepath, prompts_list, query_type, function_name):
        ...
        if query_type == "cli_fact_cy":
            documento_tipo = "cli_fact_cy"
            generate_fact_current_year_client(generated_lines, table_name, function_name, prompts_list)

        elif query_type == "cliente_fact_total":
            documento_tipo = "cliente_fact_total"
            generate_fact_total_client(generated_lines, table_name, function_name, prompts_list)

        elif query_type == "cliente_ing_anual":
            documento_tipo = "cliente_ing_anual"
            generate_ing_current_year_client(generated_lines, table_name, function_name, prompts_list)

        elif query_type == "cliente_ing_total":
            documento_tipo = "cliente_ing_total"
            generate_ing_total_client(generated_lines, table_name, function_name, prompts_list)

        elif query_type == "fact_total":
            documento_tipo = "fact_total"
            generate_fact_total(generated_lines, table_name, function_name, prompts_list)

        elif query_type == "fact_currenty":
            documento_tipo = "fact_currenty"
            generate_fact_current_year(generated_lines, table_name, function_name, prompts_list)

        elif query_type == "fact_selecty":
            documento_tipo = "fact_selecty"
            generate_fact_selected_year(generated_lines, table_name, function_name, prompts_list)

        elif query_type == "ing_total":
            documento_tipo = "ing_total"
            generate_ing_total(generated_lines, table_name, function_name, prompts_list)

        elif query_type == "ing_currenty":
            documento_tipo = "ing_currenty"
            generate_ing_current_year(generated_lines, table_name, function_name, prompts_list)

        elif query_type == "ing_selecty":
            documento_tipo = "ing_selecty"
            generate_ing_selected_year(generated_lines, table_name, function_name, prompts_list)
