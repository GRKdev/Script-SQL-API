...
class TrainingTab:
    def __init__(self, master):
        ...
        self.upload_button = tk.Button(self.frame, text="Subir Archivos", command=self.upload_and_get_ids)
        ...
        self.final_button = tk.Button(self.frame, text="Enviar", command=self.send_final_query)
        ...
        self.status_button = tk.Button(self.frame, text="Estado", command=self.check_status)
        self.status_button.grid(row=3, column=3, pady=20)
        self.status_button.config(width=10, height=2)

        self.cancel_button = tk.Button(self.frame, text="Cancelar", command=self.cancel_ft)
        ...
        self.check_selected_button = tk.Button(self.frame, text="Verificar Estado", command=self.check_selected_status)
        self.check_selected_button.grid(row=5, column=3, pady=20)
        self.check_selected_button.config(width=11, height=2)

        self.tree = ttk.Treeview(self.frame, columns=("ID", "Estado", "Costo", "Modelo Fine-Tuned"))
    ...
    def upload_and_get_ids(self):
        ...
        if not self.api_key:
            tk.messagebox.showerror("Error", "Por favor, introduce la clave API de OpenAI.")
            return

        try:
            train_response = subprocess.check_output(["openai", "api", "files.create", "-f", "Documents/dicc/results/train.jsonl", "-p", "fine-tune"])
            train_data = json.loads(train_response)
            train_id = train_data["id"]

            valid_response = subprocess.check_output(["openai", "api", "files.create", "-f", "Documents/dicc/results/valid.jsonl", "-p", "fine-tune"])
            ...
            tk.messagebox.showinfo("IDs obtenidos", f"Train ID: {train_id}\nValid ID: {valid_id}")
    ...
    def send_final_query(self):
        ...
        response = subprocess.check_output(["openai", "api", "fine_tunes.create", "-t", self.train_id, "-v", self.valid_id, "-m", "ada", "--suffix", message], text=True)
        print(response)
        ...
        for line in response.splitlines():
            ...
            tk.messagebox.showerror("Error", "No se encontró la ID de Fine-Tune en la respuesta.")
    ...
    def cancel_ft(self):
        if not hasattr(self, 'fine_tune_id'):
            messagebox.showerror("Error", "No has iniciado ningún entrenamiento recientemente.")
            return
    ...
    def check_status(self):
        ...
        response = subprocess.check_output(["openai", "api", "fine_tunes.get", "-i", self.fine_tune_id], text=True)
        ...
        for file_type, files in [("Archivo de Entrenamiento", data.get("training_files", [])), 
    ...
    def check_selected_status(self):
        ...
        if not selected_id:
            messagebox.showerror("Error", "No se encontró una ID para el mensaje seleccionado.")
            return

        response = subprocess.check_output(["openai", "api", "fine_tunes.get", "-i", selected_id], text=True)
        ...
        cost = next((event["message"].split("$")[-1] for event in data["events"] if "Fine-tune costs" in event["message"]), "N/A")
        ...
        if error_messages:
            ...
            self.tree.insert("", "end", values=(selected_id, data["status"], cost, fine_tuned_model))
